...
export const updateTaskTag = async (req, res) => {
  const { task_id, tag_id } = req.body;
  try {
    const [updated] = await TaskTagsModel.update(
      { task_id, tag_id },
    ...
    if (updated === 0) {
      return res
        .status(404)
        .json({ Message: "La relacion de etiqueta y tarea no existe" });
    }
    res
      .status(200)
      .json({ Message: "Se actualizo una relación de etiqueta y tarea" });
  } catch (error) {
...
export const deleteTaskTag = async (req, res) => {
  try {
    const deleted = await TaskTagsModel.destroy({
      where: { id: req.params.id },
    });
    if (deleted === 0)
      return res.status(404).json({ Message: "La relacion no fue encontrada" });
    res
      .status(200)
      .json({ Message: "Relacion entre tarea y etiqueta eliminada." });
  } catch (error) {
    res.status(500).json({ Message: error.message });
  }
};
